...
private:
#ifdef FAST_ADC_READ
  bool useFixedFreq = true; //true использовать фиксированное семплирование, либо максимально возможное (false)
#else
  ...
  float scale = 1.25; // 400 как средняя точка у меня, но надо будет калибравать для каждого случая отдельно калибровкой :)
  float noise = 0; // заполняется калибровкой, это уровень шума микрофона
  double signalFrequency = 700;    // 700
  ...
  const uint8_t amplitude = 100;   //100
  float *vReal = nullptr;
  float *vImag = nullptr;
  uint8_t minPeak = 0;
  uint8_t maxPeak = 0;
...
public:
#ifdef FAST_ADC_READ
  static const uint16_t samples=64U;     //256 This value MUST ALWAYS be a power of 2
#else
